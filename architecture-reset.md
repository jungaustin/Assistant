...
- [~] **Hard cap of `MAX_UTTERANCE_SECONDS` — intentionally not shipped (2026-06-08).** First attempt wired `threading.Timer` to RealtimeSTT's `on_recording_start` / `on_recording_stop` callbacks. Unit tests against a fake recorder passed (7 cases). On a real mic with `MAX_UTTERANCE_SECONDS=5` and a 12-second test utterance, **the cap did not fire** — likely because RealtimeSTT doesn't honor late-assignment of `on_recording_start` after `AudioToTextRecorder` is constructed. Could be fixed by routing the callback through the constructor, but: in evaluating the actual user need, the cap is belt-and-suspenders for failure modes (VAD wedging, ambient noise after wake, unattended pickup) that don't bite at hobby scale in a personal-use desk robot. RealtimeSTT's VAD is solid; you can always interrupt with the wake word. Revisit if a real failure shows up, or with Phase 8's Pi/hardware-mute combo where unattended capture risk goes up.
